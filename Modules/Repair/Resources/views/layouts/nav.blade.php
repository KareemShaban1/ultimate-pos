		...
		<div class="container-fluid">
			...
			<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
				<ul class="nav navbar-nav">
					...
					<li @if(request()->segment(2) == 'job-sheet' &&
						empty(request()->segment(3))) class="active" @endif>
						<a
							href="{{action('\Modules\Repair\Http\Controllers\JobSheetController@index')}}">
					...
					<li @if(request()->segment(2) == 'job-sheet' && request()->segment(3)
					...
					@can('repair.view')
					<li @if(request()->segment(2) == 'repair' &&
						empty(request()->segment(3))) class="active" @endif><a
							href="{{action('\Modules\Repair\Http\Controllers\RepairController@index')}}">@lang('repair::lang.list_invoices')</a>
					</li>
					@endcan

					@can('repair.create')
					<li @if(request()->segment(2) == 'repair' && request()->segment(3) ==
						'create') class="active" @endif><a
							href="{{ action([\App\Http\Controllers\SellPosController::class, 'create']). '?sub_type=repair'}}">@lang('repair::lang.add_invoice')</a>
					</li>
					@endcan
					...
					<li @if(request()->segment(1) == 'brands') class="active" @endif><a
							href="{{action('\App\Http\Controllers\BrandController@index')}}">@lang('brand.brands')</a>
					</li>
					@endif

					@if (auth()->user()->can('edit_repair_settings'))
					<li @if(request()->segment(1) == 'repair' && request()->segment(2) ==
						'repair-settings') class="active" @endif><a
